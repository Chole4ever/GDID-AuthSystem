...
public class GenerateChartUtil {
    ...
    public static JFreeChart createBarChart(String chartTitle, List<String> legendNameList, List<String> xAxisNameList
        ...
        // 对柱子进行渲染
        JFreeChartUtil.setBarRenderer(chart.getCategoryPlot(), false);
...
//                customBarRenderer.setDefaultItemLabelGenerator(new CustomLabelGenerator());}
//        }
        //设置系列柱体背景颜色
    ...
    static class CustomLabelGenerator extends StandardCategoryItemLabelGenerator {
        @Override
        public String generateLabel(CategoryDataset dataset, int row, int column) {
            Number value = dataset.getValue(row, column);
            if (value != null && value.doubleValue() != 0) {
                // 格式化数字，这里可以使用 NumberFormat 或者其他格式化逻辑
                return super.generateLabel(dataset, row, column);
            } else {
                return "";  // 返回空字符串代表不显示标签
            }
    ...
    public static JFreeChart createLineChart(String chartTitle, List<String> legendNameList, List<String> xAxisNameList
        ...
        CustomLineAndShapeRenderer renderer = new CustomLineAndShapeRenderer();
    ...
    static class CustomLineAndShapeRenderer extends LineAndShapeRenderer {
        @Override
        public void drawItem(Graphics2D g2, CategoryItemRendererState state, Rectangle2D dataArea,
            ...
            Number yValue = dataset.getValue(row, column);
            if (yValue != null && yValue.doubleValue() == 0) {
                return;  // Skip drawing this item if value is 0
            }

            if (pass == 1 && column > 0) {  // Only consider drawing lines in the second pass
                Number previousYValue = dataset.getValue(row, column - 1);
                if (previousYValue != null && previousYValue.doubleValue() == 0) {
                    return;  // Skip drawing line if previous value is 0
                }
            }
