...
import static com.example.authproject.ChartsUtils.JFreeChartTest.imagePath;
...
//        }
    ...
    public static void testBarChart(List<String> xAxisNameList,List<String> legendNameList,List<List<Object>> dataList) throws Exception {
        /*
        List<String> xAxisNameList = new ArrayList<>(Arrays.asList("一级", "二级", "三级", "四级", "五级"));
        //图例名称列表
        List<String> legendNameList = new ArrayList<>(Arrays.asList("李四", "张三"));
        ...
        //图例背景颜色
        List<Color> legendColorList = new ArrayList<>(Arrays.asList(new Color(65, 105, 225)));
        JFreeChart chart = GenerateChartUtil.createBarChart("Threshold", legendNameList, xAxisNameList
                , dataList, JFreeChartUtil.createChartTheme("宋体"), "Latency/ms", "Number of UAVs",
                0d, 100d, legendColorList, true, "{2}");
        //在D盘目录下生成图片
        File p = new File(imagePath);
        if (!p.exists()) {
